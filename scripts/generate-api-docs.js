  ...
  if (operation.requestBody) {
    ...
    if (requestContent) {
      ...
      if (schema) {
        ...
        const exampleBody = getRequestBodyExample(operation, spec)
        if (exampleBody) {
          content += '**Example:**\n\n'
          content += '```json\n'
          content += JSON.stringify(exampleBody, null, 4) + '\n'
          content += '```\n\n'
        }
      }
    }
...
/**
 * Get request body example from operation
 * @param {Object} operation - OpenAPI operation object
 * @param {Object} spec - Full OpenAPI spec for resolving refs
 * @returns {Object|null} Example request body object
 */
...
function generateCodeSamples(endpoint, spec, parameters) {
  ...
  const requestBody = getRequestBodyExample(operation, spec)
  ...
  for (const [statusCode, response] of Object.entries(operation.responses)) {
    ...
    if (response.content) {
      ...
      if (responseContent?.schema) {
        ...
        if (exampleData !== null) {
          body = JSON.stringify(exampleData, null, 4)
        }
      }
    }
...
async function updateNavigationConfig(groupedEndpoints) {
  // Build navigation items for generated endpoints with sub-items for each endpoint
  const navItems = []
  for (const [tag, group] of Object.entries(groupedEndpoints)) {
    const slug = tagToSlug(tag)

    // Build sub-items for each endpoint within this tag
    const endpointItems = group.endpoints.map(endpoint => {
      const endpointSlug = tagToSlug(endpoint.operation.summary || endpoint.path)
      return {
        title: endpoint.operation.summary || endpoint.path,
        href: `/docs/api/${slug}#${endpointSlug}`,
...
function buildNavigationFile(apiNavItems) {
  // Helper function to format a nav item with optional method and sub-items
  function formatNavItem(item, indentLevel = 10) {
    const indent = ' '.repeat(indentLevel)
    let result = `${indent}{\n`
    result += `${indent}  title: '${item.title}',\n`

    if (item.href) {
      result += `${indent}  href: '${item.href}',\n`
    }

    if (item.method) {
      result += `${indent}  method: '${item.method}',\n`
    }

    if (item.items && item.items.length > 0) {
      result += `${indent}  items: [\n`
      result += item.items.map(subItem => formatNavItem(subItem, indentLevel + 4)).join(',\n')
      result += `\n${indent}  ],\n`
    }
  ...
  const formattedItems = apiNavItems.map(item => formatNavItem(item, 10)).join(',\n')

  return `// autogenerated from generate-api-docs.js
